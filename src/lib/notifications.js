  ...
  try {
    ...
    for (const item of pendingList) {
      try {
        ...
        const subject = `[IT-Helpdesk] Neue Nachricht(en) zu Ticket ${item.ticketId}: ${ticketTitle}`;
        ...
        // Nachrichtentext für Mail aufbauen
        const messagesHtml = msgItems.map(m => `
          <div style="background-color: #0f172a; border-left: 4px solid #8b5cf6; padding: 12px; margin-bottom: 10px; border-radius: 6px;">
            <div style="font-weight: bold; color: #38bdf8; font-size: 12px; margin-bottom: 4px;">
              ${m.senderName} <span style="color: #64748b; font-weight: normal;">(${m.time} Uhr)</span>
            </div>
            <div style="color: #f1f5f9; font-size: 14px; white-space: pre-wrap; line-height: 1.5;">${m.text}</div>
          </div>
        `).join('');
        ...
        const html = `
          <div style="font-family: sans-serif; background-color: #020617; color: #f8fafc; padding: 24px; border-radius: 12px; max-width: 600px; margin: 0 auto; border: 1px solid #1e293b;">
            ...
            <p style="color: #94a3b8; font-size: 14px; margin-bottom: 20px;">
              Es gibt <strong>${msgItems.length} neue Nachricht(en)</strong> zu deinem Anliegen <em>"${ticketTitle}"</em>:
            </p>

            ${messagesHtml}

            <div style="margin-top: 24px; text-align: center;">
              <a href="${ticketUrl}" style="background-color: #7c3aed; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 14px; display: inline-block;">
