...
export async function flushPendingNotifications() {
  try {
    ...
    for (const item of pendingList) {
      await processAndSendNotificationItem(item);
    }
  } catch (err) {
...
/**
 * Flasht sofort alle ausstehenden Benachrichtigungen für ein bestimmtes Ticket (z. B. vor Ticket-Schließung).
 */
export async function flushTicketNotificationsNow(ticketId) {
  if (!ticketId) return;
  try {
    const pendingList = db.prepare(`
      SELECT id, ticket_id as ticketId, recipient_email as recipientEmail, 
             ...
             created_at as createdAt 
      FROM pending_ticket_notifications 
      WHERE ticket_id = ? AND sent_at IS NULL
    `).all(ticketId);

    if (pendingList.length === 0) return;

    console.log(`[Notification-Queue] Flashe sofort ${pendingList.length} gepufferte Nachricht(en) für Ticket ${ticketId}...`);

    for (const item of pendingList) {
      await processAndSendNotificationItem(item);
    }
  } catch (err) {
    console.error(`[Notification-Queue] Fehler bei flushTicketNotificationsNow für Ticket ${ticketId}:`, err);
  }
}
